...
function EFFECT:Init(data)
	...
	if (sparticle) then
		...
		sparticle:SetEndSize(24 * (halofac * 0.8 + 0.2), 0, 1)
		local r = math.Rand(-10, 10) * 3.14 / 180
		sparticle:SetRoll(r)
		sparticle:SetRollDelta(r / 5)
		sparticle:SetColor(255, 255, 192)
	...
	for i = 1, 1 do
		...
		if (particle) then
			...
			particle:SetDieTime(0.20)
			particle:SetStartAlpha(math.Rand(225, 255))
			particle:SetEndAlpha(0)
			--particle:SetStartSize( 7.5 * (halofac*0.8+0.2), 0, 1)
			--particle:SetEndSize( 0 )
			particle:SetStartSize(12 * (halofac * 0.8 + 0.2), 0, 1)
			particle:SetEndSize(24 * (halofac * 0.8 + 0.2))
	...
	for i = 0, 4 do
		...
		if (particle) then
			...
			particle:SetLifeTime(0)
			particle:SetDieTime(math.Rand(0.12, 0.14))
			particle:SetStartAlpha(math.Rand(7, 11))
			particle:SetEndAlpha(0)
			particle:SetStartSize(math.Rand(10, 14))
			particle:SetEndSize(math.Rand(24, 28))
			particle:SetRoll(math.rad(math.Rand(0, 360)))
	...
	if TFA.GetGasEnabled() then
		for i = 0, 3 do
			...
			if (particle) then
				...
				particle:SetLifeTime(0)
				particle:SetDieTime(math.Rand(0.1, 0.30))
