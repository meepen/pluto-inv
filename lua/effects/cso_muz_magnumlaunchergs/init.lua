...
function EFFECT:Init(data)
	...
	if (sparticle) then
		...
		sparticle:SetStartSize(6 * (halofac * 0.8 + 0.2), 0, 1)
		sparticle:SetEndSize(18 * (halofac * 0.8 + 0.2), 0, 1)
		local r = math.Rand(-10, 10) * 3.14 / 180
		sparticle:SetRoll(r)
		sparticle:SetRollDelta(r / 5)
		sparticle:SetColor(218, 141, 255)
	...
	for i = 1, 1 do
		...
		if (particle) then
			...
			particle:SetEndSize(18 * (halofac * 0.8 + 0.2))
			particle:SetRoll(math.rad(math.Rand(0, 360)))
			particle:SetRollDelta(math.rad(math.Rand(-40, 40)))
			particle:SetColor(218, 141, 255)
	...
	for i = 0, 5 do
		...
		if (particle) then
			...
			--particle:SetEndSize( 0 )
			particle:SetStartSize(math.max(8 - 0.65 * i, 1) * 0.55)
			particle:SetEndSize(math.max(7.2 - 0.55 * i, 1) * 1.25)
			particle:SetRoll(math.rad(math.Rand(0, 360)))
			particle:SetRollDelta(math.rad(math.Rand(-40, 40)))
			particle:SetColor(255, 239, 171)
			particle:SetLighting(false)
			particle.FollowEnt = self.WeaponEnt
			particle.Att = self.Attachment
			TFA.Particles.RegisterParticleThink(particle, TFA.Particles.FollowMuzzle)
	...
	for i = 0, 4 do
		...
		if (particle) then
			...
			particle:SetEndSize(math.Rand(15, 18))
			particle:SetRoll(math.rad(math.Rand(0, 360)))
