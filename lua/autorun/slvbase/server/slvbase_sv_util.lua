...
function util.BlastDmg(inflictor, attacker, pos, radius, dmg, TFilter, dmgType, bReduce)
	...
	for k, v in ipairs(ents.FindInSphere(pos, radius)) do
		if (!TFilter || (type(TFilter) == "table" && !table.HasValue(TFilter, v)) || type(TFilter) != "table" && TFilter(v)) && !util.TraceLine({start = pos, endpos = v:GetPos() +v:OBBCenter(), mask = MASK_NPCWORLDSTATIC}).Hit && (!v:IsPlayer() || !v:SLVIsPossessing()) then
			...
			if (v:IsNPC() || v:IsPlayer()) && dmgType == DMG_ENERGYBEAM && v:Health() <= 0 then v:slvDissolve(attacker, inflictor, 0) end
...
function util.DealBlastDamage(pos,radius,dmg,force,attacker,inflictor,bReduce,dmgType,TFilter,fcOnHit)
	...
	for _, ent in ipairs(ents.FindInSphere(pos,radius)) do
		if (!TFilter || (type(TFilter) == "table" && !table.HasValue(TFilter, ent)) || type(TFilter) != "table" && TFilter(ent)) && !util.TraceLine({start = pos, endpos = ent:GetPos() +ent:OBBCenter(), mask = MASK_NPCWORLDSTATIC}).Hit && (!ent:IsPlayer() || !ent:SLVIsPossessing()) then
			...
			if(!fcOnHit || !fcOnHit(ent,dmgInfo)) then
				if (ent:IsNPC() || ent:IsPlayer()) && dmgType == DMG_ENERGYBEAM && ent:Health() <= 0 then ent:slvDissolve(attacker,inflictor, 0) end
			end
...
function util.SetRelationshipsThroughClass(class,entity)
	if table.HasValue(tblNPCClasses[class],entity) then
		entity.iClass = class
	end
end
