...
function meta:SetFrozen(iPercent)
	if(self:SLVIsPossessing()) then return end
	iPercent = iPercent || 100
	iPercent = gamemode.Call("ScalePlayerFreeze",self,iPercent) || iPercent
	if(iPercent > 0 && self:slvIsOnFire()) then self:slvExtinguish() end
...
function meta:slvIsFrozen()
	return self.bFrozen
end

meta = FindMetaTable("NPC")
function meta:SLVPlayNPCGesture(seq,layer,playbackrate)
...
function meta:HasCapability(cap) return bit.band(self:slvCapabilitiesGet(),cap) == cap end

function meta:slvCapabilitiesGet() -- Default function is broken, so we're doing a workaround; Only works for SNPCs
	return self.m_iCapabilities || 0
end

local CapabilitiesAdd = meta.CapabilitiesAdd
function meta:slvCapabilitiesAdd(...)
...
function meta:slvAddEntityRelationship(...)
	local ent, disp, priority = ...
	local dispLast = self:slvDisposition(ent)
	if !ent.bScripted && ent:GetNoTarget() && !ent:IsPlayer() then
		tblNoTargetNPCs[ent][self] = disp
		gamemode.Call("OnRelationshipChanged",self,ent,self:slvDisposition(ent),dispLast)
		return
	end
...
function meta:slvSetNoTarget(bNoTarget)
	if bNoTarget then
		if !table.HasValue(tblNoTargetNPCs, self) then
			...
			for k, ent in pairs(ents.GetNPCs()) do
				if !ent.bScripted && ent != self then
					tblNoTargetNPCs[self][ent] = ent:slvDisposition(self)
					slvAddEntityRelationship(ent, self, D_NU, 100)
				end
			end
		end
		return
	...
	for ent, disp in pairs(tblNoTargetNPCs[self]) do
		if IsValid(ent) then
			slvAddEntityRelationship(ent, self, disp, 100)
		end
	end
	...
	if IsValid(ent) && ent:IsNPC() && !ent.bScripted then
		for _ent, rel in pairs(tblNoTargetNPCs) do
			if IsValid(_ent) then
				tblNoTargetNPCs[_ent][ent] = ent:slvDisposition(_ent)
				slvAddEntityRelationship(ent, _ent, D_NU, 100)
...
function meta:slvAddVelocity(vel)
...
function meta:slvSetSpawnEffect(...)
	local b = ...
...
local slvDisposition = meta.Disposition
function meta:slvDisposition(...)
	local ent = ...
	return tblEntRelationshipExcept[self] && tblEntRelationshipExcept[self][ent] || slvDisposition(self, ...)
end
...
function meta:slvIsOnFire()
	return tblEntsIgnited[self] != nil || IsOnFire(self)
end

function meta:slvFadeOut(fDelay)
...
function meta:slvFadeIn(fDelay)
...
function meta:slvDissolve(entAttacker, entInflictor, iType)
	...
	entDissolver:Fire("slvDissolve", sName, 0)
...
meta = FindMetaTable("Player")
function meta:Irradiate(ent,RAD)
	local dmg = DamageInfo()
...
hook.Add("PlayerSpawn", "SVL_PlayerSpawn", function(ply)
	...
	if ply:slvIsOnFire() then ply:slvExtinguish() end
end)
...
function meta:SLVSetAngleVelocity(ang)
