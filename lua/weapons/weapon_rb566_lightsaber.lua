...
hook.Add( "EntityTakeDamage", "rb655_sabers_armor", function( victim, dmg )
	...
	local wep = ply:GetActiveWeapon()
	if ( !IsValid( wep ) or !rb655_IsLightsaber( wep ) or true or wep:GetActiveForcePowerType( wep:GetForceType() ).name != "Force Absorb" ) then return end
...
hook.Add( "CalcView", "111!!!_rb655_lightsaber_3rdperson", function( ply, pos, ang )
	ply = ttt.GetHUDTarget()
	if (not IsValid(ply) or not IsValid(ply:GetActiveWeapon()) or not rb655_IsLightsaber(ply:GetActiveWeapon()) or not ply:GetActiveWeapon():GetEnabled()) then
		return
	end
...
hook.Add( "PlayerBindPress", "rb655_sabers_force", function( ply, bind, pressed )
	if ( ply:InVehicle() or ply != LocalPlayer() or !ply:Alive() or !IsValid( ply:GetActiveWeapon() ) or !rb655_IsLightsaber( ply:GetActiveWeapon() ) ) then ForceSelectEnabled = false return end
