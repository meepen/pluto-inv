...
SWEP.PrintName				= "Dual Uzi Expert (B Mode)"		-- Weapon name (Shown on HUD)	
...
--Firing related
SWEP.Primary.Sound 			= Sound("DualUzi.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.RPM				= 1400					-- This is in Rounds Per Minute / RPM
SWEP.Primary.RPM_Semi				= 1400					-- RPM for semi-automatic or burst fire.  This is in Rounds Per Minute / RPM
SWEP.FiresUnderwater = true

-- nZombies Stuff
SWEP.NZWonderWeapon		= false	-- Is this a Wonder-Weapon? If true, only one player can have it at a time. Cheats aren't stopped, though.
--SWEP.NZRePaPText		= "your text here"	-- When RePaPing, what should be shown? Example: Press E to your text here for 2000 points.
SWEP.NZPaPName				= "The Matrix"
...
SWEP.Primary.ClipSize			= 98					-- This is the size of a clip
SWEP.Primary.DefaultClip			= 588				-- This is the number of bullets the gun gives you, counting a clip as defined directly above.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_dualuzi_v6_stock.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_dualuzi_v6_r.mdl" -- Worldmodel path
...
SWEP.WElements = {
	["dual"] = { type = "Model", model = "models/weapons/tfa_cso/w_dualuzi_v6_l.mdl", bone = "ValveBiped.Bip01_L_Hand", rel = "", pos = Vector(4.079, 1.292, 3), angle = Angle(0, 81.939, 0.534), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:ChooseShootAnim(ifp)
	if self.Akimbo and self:GetActivityEnabled( ACT_VM_SECONDARYATTACK ) and ((self.AnimCycle == 0 and not self.Akimbo_Inverted) or (self.AnimCycle == 1 and self.Akimbo_Inverted)) then
		if self:GetSilenced() then print("shit")
			--return TFA.Enum.ANIMATION_ACT, ACT_VM_DRYFIRE_SILENCED
		end
	end
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_dualuzi_v6_stock")
