...
SWEP.Type	= "Unique Grade Equipment"

--[[WEAPON HANDLING]]--

--Firing related
SWEP.Primary.Sound 			= Sound("Ballista.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_ballista.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_ballista.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_ballista" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_ballista" 	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Secondary.Sound = Sound( "Ballista.Fire2" )
...
if self.Secondary.Nextregen <= CurTime()  and self:Ammo2() < 10 then 
 ...
 self:EmitSound("Ballista.Missile_Reload")
end
...
local vel = self.Owner:GetForward() * 5000 - self.Owner:GetRight()  * 400
for i=5,1,-1 do  --i = 9 - 9 shoots
local ent = ents.Create("ballista_missile")
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_ballista")
