...
SWEP.PrintName				= "Magnum Launchcer Global Showcase 2018"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("MagnumLancer.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_magnumlauncher_gs18.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_magnumlauncher_gs18.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_magnumlaunchergs" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_magnumlaunchergs" 	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Secondary.Ammo			= "BULLET_LANCER" -- Self explanitory, ammo type.
SWEP.Secondary.Sound = Sound( "MagnumLancer.Fire2" )

SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_magnumlauncher_gs18")
...
if self.Secondary.Nextregen <= CurTime()  and self:Ammo2() < 10 then 

  self.Owner:GiveAmmo( 1,"BULLET_LANCER", true )                     -------SECONDARY AMMO REGEN
 self.Secondary.Nextregen =  CurTime()  + 1.5   ---3 seconds
 self:EmitSound("MagnumLancer.MissileReload")
end
...
function SWEP:CanSecondaryAttack()
    if self:GetSprinting() and not self:GetStat("AllowSprintAttack", false) then return false end
...
local vel = self.Owner:GetForward() * 5000 - self.Owner:GetRight()  * 800
for i=9,1,-1 do  --i = 9 - 9 shoots
local ent = ents.Create("sg_missilegs")
...
vel = vel + self.Owner:GetRight()  * 200
end
self:SetNextSecondaryFire(CurTime() + 0.5)
self:TakeSecondaryAmmo(1)
self.Weapon:SendWeaponAnim(ACT_VM_SECONDARYATTACK)
self:EmitSound( self:GetStat("Secondary.Sound") )
 self.Secondary.Nextregen =  CurTime()  + 1.5
