...
SWEP.Distance = 250
SWEP.Winks = 1
SWEP.RechargeRate = 0.25 / SWEP.Winks
...
--[[SWEP.Equipment = {
	Name		   = "Wink",
	Desc 		   = "In a wink of an eye, they were... still there.",
...
function SWEP:SetupDataTables()
	...
	self:NetVar("Winking", "Bool", false)
	self:NetVar("ToPos", "Vector", none)
	self:NetVar("Charge", "Float", 1)
	self:NetVar("LastWink", "Float", -math.huge)
	self:NetVar("NeedStop", "Bool", false)
end

function SWEP:GetCurrentCharge()
	return math.Clamp(self:GetCharge() + (CurTime() - self:GetLastWink()) * self.RechargeRate, 0, 1)
end
...
function SWEP:DrawHUD()
	...
	if (self:GetCurrentCharge() >= 1 / self.Winks) then
	...
	if (self:GetWinking()) then
		...
		if (not own:KeyDown(IN_ATTACK)) then
			...
			if (pos) then
				...
				self:SetToPos(pos)
				self:SetCharge(math.max(0, self:GetCurrentCharge() - 1 / self.Winks))
				self:SetLastWink(CurTime())
				if (SERVER) then
					RagdollDissolveEffect(self:GetOwner())
				end
			end
		end
	elseif (self:GetNeedStop()) then
		self:SetNeedStop(false)
...
function SWEP:PrimaryAttack()
	if (self:GetWinking() or self:GetToPos() ~= none or self:GetCurrentCharge() < 1 / self.Winks) then
		return
	end
	local own = self:GetOwner()
	...
	self:SetWinking(true)
	self:SetNeedStop(true)
...
function SWEP:Holster()
	if (self:GetWinking() or self:GetToPos() ~= none) then
		return false
	end
