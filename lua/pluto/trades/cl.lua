...
--[[
	...
	for _, trade in ipairs(trades) do
		list:AddLine(trade.ID, trade.p1name, trade.p2name)
	end
...
function PANEL:Init()
	...
	self.ChatLog = self.Inner:Add "RichText"
	function self.ChatLog:PerformLayout()
		self:SetFontInternal "pluto_trade_chat_bold"
	end
	self.ChatLog:Dock(FILL)
	self.ChatLog:SetFontInternal "pluto_trade_chat_bold"

	self.Player1 = self:Add "pluto_trade_snapshot_info"
	self.Player1:Dock(TOP)
	self.Player2 = self:Add "pluto_trade_snapshot_info"
	self.Player2:Dock(TOP)
end

function PANEL:SetTrade(trade)
	self:SetTitle("Trade ID " .. trade.ID)

	local snapshot = json.parse(trade.snapshot)

	local chat = self.ChatLog
	local lookup = {
		[trade.p1steamid] = trade.p1name,
		[trade.p2steamid] = trade.p2name,
	}

	for _, msg in ipairs(snapshot.messages) do
		local col = ttt.teams.innocent.Color
		chat:InsertColorChange(col.r, col.g, col.b, col.a)
		chat:AppendText(lookup[msg[1]])
		col = white_text
		chat:InsertColorChange(col.r, col.g, col.b, col.a)
		chat:AppendText ": "
		chat:AppendText(msg[2])
		chat:AppendText "\n"
	end

	self.Player1:SetInfo(trade.p1name, trade.p1steamid)
	self.Player1:SetCurrencies(snapshot.currencies[trade.p1steamid])
	self.Player1:SetItems(snapshot.items[trade.p1steamid])
	self.Player2:SetInfo(trade.p2name, trade.p2steamid)
	self.Player2:SetCurrencies(snapshot.currencies[trade.p2steamid])
	self.Player2:SetItems(snapshot.items[trade.p2steamid])
	PrintTable(snapshot.items)
end

vgui.Register("pluto_trade_log", PANEL, "DFrame")
...
function PANEL:Init()
	...
	self.Label:SetFont "pluto_trade_chat_bold"
	self.CurrencyCarrier = self:Add "EditablePanel"
	self.CurrencyCarrier:Dock(TOP)
	self.CurrencyCarrier:SetTall(80)
	self.ItemCarrier = self:Add "EditablePanel"
	self.ItemCarrier:Dock(TOP)
	self.ItemCarrier:SetTall(48)
	self.ItemCarrier2 = self:Add "EditablePanel"
	self.ItemCarrier2:Dock(TOP)
	self.ItemCarrier2:SetTall(48)


	self.Items = {}
	for i = 1, 6 do
		local p = self.ItemCarrier:Add "pluto_inventory_item"
		function p:SwitchWith()
		end
		p.NoMove = true
		p.TabIndex = 0
		p:Dock(LEFT)
		p:SetWide(p:GetParent():GetTall())
		self.Items[i] = p
	end
	for i = 7, 12 do
		local p = self.ItemCarrier2:Add "pluto_inventory_item"
		function p:SwitchWith()
		end
		p.NoMove = true
		p.TabIndex = 0
...
function PANEL:SetCurrencies(currencies)
	for _, cur in pairs(self.Currencies) do
		cur:Remove()
	end
	...
	for what, amt in pairs(currencies) do
		local cur = self.CurrencyCarrier:Add "pluto_trade_currency"
		self.Currencies[which] = cur
		self.Currencies[which]:SetInfo(what, amt)
		cur:Dock(LEFT)
		which = which + 1
	end
end
...
function PANEL:SetItems(items)
	for _, pnl in pairs(self.Items) do
		pnl:SetItem()
	end
...
function PANEL:SetInfo(name, steamid)
	self.Label:SetText(name .. "(" .. steamid .. ") traded...")
end

vgui.Register("pluto_trade_snapshot_info", PANEL, "EditablePanel")