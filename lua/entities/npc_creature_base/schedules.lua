...
function ENT:RunAI(strExp)
	...
	if !self:SLV_IsPossesed() && !self.UseActivityTranslator then
	...
	if self.m_actArrival && self:SLVIsMoving() && self:GetArrivalActivity() != self.m_actArrival then
	...
	elseif(self.UseIdleSystem && !self:SLVIsMoving()) then self:PlayIdle() end
