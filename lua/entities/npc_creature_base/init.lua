...
ENT.VanilaRelationships = {
	[CLASS_PLAYER_ALLY] = {"npc_gman", "monster_gman", "npc_alyx", "npc_barney", "npc_citizen", "npc_vortigaunt", "npc_monk", "npc_breen", "npc_dog", "npc_eli", "npc_fisherman", "monster_scientist", "monster_sitting_scientist", "npc_kleiner", "npc_magnusson", "npc_mossman", "player", "monster_barney", "npc_mmorpg_salesman", "npc_mmorpg_questman", "npc_mmorpg_civilian", "npc_libertyprime", "npc_dogmeat", "npc_dogskin", "npc_chicken", "npc_protectron", "npc_protectron_outcast", "npc_protectron_construction", "monster_otis", "monster_rosenberg", "monster_wheelchair"},
	[CLASS_COMBINE] = {"npc_combine_s", "npc_cscanner", "npc_hunter", "npc_manhack", "npc_mortarsynth", "npc_rollermine", "npc_turret_floor", "npc_clawscanner", "npc_combine_camera", "npc_combinedropship", "npc_combinegunship", "npc_helicopter", "npc_turret_ceiling", "npc_turret_ground", "npc_combine_assassin", "npc_strider"},
	[CLASS_MILITARY] = {"monster_human_grunt", "monster_hwgrunt", "monster_human_assassin", "monster_sentry", "monster_miniturret", "monster_miniturret_ceiling", "monster_sentry_decay"},
	[CLASS_ZOMBIE] = {"npc_headcrab", "npc_headcrab_black", "npc_headcrab_poison", "npc_headcrab_fast", "monster_babycrab", "monster_headcrab", "monster_bigmomma", "monster_zombie_barney", "monster_zombie_soldier", "monster_zombie", "npc_fastzombie_torso", "npc_fastzombie",  "npc_poisonzombie", "npc_zombie", "npc_zombie_torso", "npc_zombine", "monster_gonome", "npc_zombine_torso"}
}

hook.Add("OnEntityCreated","SLVBase_CreateVanilaRelationships",function(ent)
	tblPlayerAlly = {"npc_gman", "monster_gman", "npc_alyx", "npc_barney", "npc_citizen", "npc_vortigaunt", "npc_monk", "npc_breen", "npc_dog", "npc_eli", "npc_fisherman", "monster_scientist", "monster_sitting_scientist", "npc_kleiner", "npc_magnusson", "npc_mossman", "player", "monster_barney", "npc_mmorpg_salesman", "npc_mmorpg_questman", "npc_mmorpg_civilian", "npc_libertyprime", "npc_dogmeat", "npc_dogskin", "npc_chicken", "npc_protectron", "npc_protectron_outcast", "npc_protectron_construction", "monster_otis", "monster_rosenberg", "monster_wheelchair"}
	tblCombine = {"npc_combine_s", "npc_cscanner", "npc_hunter", "npc_manhack", "npc_mortarsynth", "npc_rollermine", "npc_turret_floor", "npc_clawscanner", "npc_combine_camera", "npc_combinedropship", "npc_combinegunship", "npc_helicopter", "npc_turret_ceiling", "npc_turret_ground", "npc_combine_assassin", "npc_strider"}
	tblMilitary = {"monster_human_grunt", "monster_hwgrunt", "monster_human_assassin", "monster_sentry", "monster_miniturret", "monster_miniturret_ceiling", "monster_sentry_decay"}
	tblZombies = {"npc_headcrab", "npc_headcrab_black", "npc_headcrab_poison", "npc_headcrab_fast", "monster_babycrab", "monster_headcrab", "monster_bigmomma", "monster_zombie_barney", "monster_zombie_soldier", "monster_zombie", "npc_fastzombie_torso", "npc_fastzombie",  "npc_poisonzombie", "npc_zombie", "npc_zombie_torso", "npc_zombine", "monster_gonome", "npc_zombine_torso"}
	if ent:IsNPC() then
		if table.HasValue(tblPlayerAlly,ent:GetClass()) then
			ent.iClass = CLASS_PLAYER_ALLY
		end
		if table.HasValue(tblCombine,ent:GetClass()) then
			ent.iClass = CLASS_COMBINE
		end
		if table.HasValue(tblMilitary,ent:GetClass()) then
			ent.iClass = CLASS_MILITARY
		end
		if table.HasValue(tblZombies,ent:GetClass()) then
...
function ENT:SetupSLVFactions() end

function ENT:OnKilledTarget(ent) end
...
function ENT:SLVGetFaction() return self.Faction end
...
function ENT:SetCustomMovementActivity(act)
	if self:SLVIsMoving() then self:SetMovementActivity(act) end
	self.actCMove = act
end

function ENT:SetDefaultArrivalActivity(act)
	if self:SLVIsMoving() then self:SetArrivalActivity(act) end
...
function ENT:TaskStart_TASK_CHECK_PATH_TIME()
	...
	if self.currentNodePos != nil && (!self.currentNodePos || self.currentNodePos != self.currentPath[1].pos) || (self.lastPathCheck != nil && CurTime() -self.lastPathCheck >= 0.65) then
...
function ENT:Wake()
	...
	self:slvSetNoTarget(false)
	self:SetInvincible(false)
	self:DrawShadow(true)
	if self.m_iKDMovetype != nil then
...
function ENT:StandUp()
	...
	self:SLVPlayActivity(self.m_actGetUp)
	self:OnStandUp()
end
...
function ENT:RunDefaultBehavior(bIgnoreEnemies)
	...
	if (!bGuard && (bIgnoreEnemies || (iState != NPC_STATE_COMBAT && (self.iBehavior != 1 || self.iMemCount == 0) && !self:HearEnemy()))) || self.bInSchedule then
		...
		elseif !self.m_iCustomIdle && (AIType == 2 || AIType == 3) && self.bPlayIdle && !self.bInSchedule && !self:SLVIsMoving() then
...
function ENT:SelectSchedule(iNPCState)
	...
	if self.m_iCustomIdle && !self.CurrentSchedule && !self:SLVIsMoving() && (!self.bInSchedule || self:GetActivity() == ACT_IDLE) then
...
function ENT:OnHearEnemy(ent) end

function ENT:SLVFaceEnemy()
	local pos
	if self.entEnemy == nil then
		pos = self:GetPossessor():GetPossessionEyeTrace().HitPos
	else
		pos = self.entEnemy:GetPos()
	end
...
function ENT:GetEnemies(ang)
	...
	for _, ent in ipairs(tbEnts) do
		if(ent != self && ent:Alive() && !ent:GetNoTarget() && (self:GetAIType() != AI_TYPE_WATER || ent:WaterLevel() > 1) && !ent.bSelfDestruct) then
			...
			if((disp == D_HT || disp == D_FR) && !gamemode.Call("NPCCanSee",self,ent) && (!ent:IsPlayer() || !ent:SLVIsPossessing()) && self:InSight(ent,ang)) then
...
function ENT:IsEnemy(ent)
	...
	-- return IsValid(ent)
	return IsValid(ent) && !ent.bNeutral && (ent:IsNPC() || ent:IsPlayer()) && !ent:GetNoTarget() && self:Disposition(ent) <= 2 && (!ent:IsPlayer() || ((GetConVarNumber("ai_ignoreplayers") == 0) && !ent:SLVIsPossessing())) && ent != self && ent:Health() > 0 && gamemode.Call("ShouldIgnoreEnemy",self,ent) != true && self:IsInAttackRange(ent)
end

// What's this for again?
function ENT:IsEnemyPlayer(ent)
	return IsValid(ent) && ((ent:IsNPC() && ent.Charmed && ent:Charmed()) || ent:IsPlayer()) && !ent:GetNoTarget() && self:Disposition(ent) <= 2 && (!ent:IsPlayer() || ((GetConVarNumber("ai_ignoreplayers") == 0) && !ent:SLVIsPossessing())) && ent != self && ent:Health() > 0
end
...
function ENT:SLVPlayActivity(TAct,bFaceEnemy,fcDone,bDontResetAct,bDontStopMoving)
	...
	if !bDontStopMoving && (self:SLVIsMoving() || (!self.bInSchedule && self.CurrentSchedule)) then
	...
	self:StartSchedule(schdAct)
	--self.CurrentSchedule.Name = "SLVPlayActivity" .. TAct
end
...
function ENT:UpdateEnemies()
	local num = self.iMemCount
	if self.entEnemy && (!IsValid(self.entEnemy) || self.entEnemy:Health() <= 0 || self.entEnemy:GetNoTarget() || !self:IsInAttackRange(self.entEnemy) || (self:GetAIType() == 5 && self.entEnemy:WaterLevel() < 2) || (self.entEnemy:IsPlayer() && (tobool(GetConVarNumber("ai_ignoreplayers")) || self.entEnemy:SLVIsPossessing())) || self.entEnemy.bSelfDestruct) then self:RemoveFromMemory(self.entEnemy) end
	self:UpdateMemory()
	self:GetEnemies()
...
-- function ENT:GetVanillaClasses(ent)
	-- local send = false
	-- PrintTable(self.VanilaRelationships)
	-- //2 - Player Ally
	-- //9 - Combine
	-- //19 - Military
	-- //64 - Zombie
	-- print(ent:GetClass())
	-- if table.HasValue(self.VanilaRelationships,self.iClass) then
		-- print("found class")
		-- for k,v in pairs(self.VanilaRelationships[self.iClass]) do
			-- print("checking ents")
...
function ENT:NewCheckRelations(ent)
	if ent:IsNPC() && ((/*(self.NPCFaction != ent.NPCFaction or ent.NPCFaction == nil) or*/ (self.iClass != ent.iClass) or ent.iClass == nil) /*or self:GetVanillaClasses(ent) == false*/) then
		-- print(self.NPCFaction,ent.NPCFaction)
		if self:Disposition(ent) != D_HT then
			-- print('bad npc!')
			self:AddEntityRelationship(ent,D_HT,100)
			ent:AddEntityRelationship(self,D_HT,100)
		end
	elseif ent:IsNPC() && ((/*(self.NPCFaction == ent.NPCFaction) or*/ (self.iClass == ent.iClass)) /*or self:GetVanillaClasses(ent) == true*/) then
		-- print(self.iClass,ent.iClass)
	...
	elseif ent:IsPlayer() && self.iClass == CLASS_PLAYER_ALLY then
		if self:Disposition(ent) != D_LI then
			self:AddEntityRelationship(ent,D_LI,100)
			-- print('friend ply!')
		end
...
function ENT:SetUpEnemies()
	...
	local faction = self.NPCFaction
	for _,ent in ipairs(ents.GetAll()) do
		if self.m_bInitialized == true then
			if((ent:IsNPC() || ent:IsPlayer()) && ent != self) then
				if !table.HasValue(self.enttbltemp,ent) then
					self:NewCheckRelations(ent)
					table.insert(self.enttbltemp,ent)
				end
			end
		end
	end
...
function ENT:PlayLayeredGesture(seq,layer,playbackrate)
...
function ENT:CreateRagdoll()
	...
	if self:slvIsOnFire() then ragdoll:slvIgnite(math.Rand(8,10),0) end
...
function ENT:OnTakeDamage(dmginfo)
	...
	if self:GetAIType() != 1 && !self:SLV_IsPossesed() && self:GetState() != NPC_STATE_COMBAT && self.iMemCount == 0 && !self.bInSchedule && entAttacker:IsValid() && self:Disposition(entAttacker) == D_HT then
	...
	if self.bInvincible || (self.m_tbInvincible && (self.m_tbInvincible[entAttacker] || self.m_tbInvincible[sAttackerClass])) || (bAttackerValid && ((entAttacker:IsNPC() && (entAttacker != self || entInflictor == self) && entAttacker:Disposition(self) >= 3 && !entAttacker:SLV_IsPossesed()) || (self.bIgnoreRagdollDamage && sAttackerClass == "prop_ragdoll"))) then bIgnoreDamage = true
	else
	...
	elseif(self.DamageScales) then
		...
		if(self.DamageScales[dmgType]) then
			if(self.DamageScales[dmgType] < 0) then
				...
				self:slvSetHealth(math.min(hp +fDmg *(self.DamageScales[dmgType] *-1),hpMax))
			end
	...
	if(GAMEMODE.NPCDamageForce) then
		...
		local ragdoll = self:GetRagdollEntity()
		if(IsValid(ragdoll)) then ragdoll:slvAddVelocity(force)
	...
	if self:Health() <= 0 then
		if(self:IsEssential()) then
			self:slvSetHealth(self:GetMaxHealth())
			self:KnockDown(12)
			return
		end
...
function ENT:DoDeath(dmginfo)
	...
	if (!IsValid(attacker) || attacker:GetClass() != "npc_barnacle") && !dmginfo:IsDamageType(DMG_DISSOLVE) then
		...
		if self.bFadeOnDeath then self:slvFadeOut(4) end
		...
		if self.bPlayDeathSequence then
			if((bRagdoll && (self.BodyCaps && true || tobool(GetConVarNumber("ai_keepragdolls")))) || self.UseRagdollPhysicsProp) then self:DoRagdollDeath(dmginfo)
				...
				local bMoving = self:SLVIsMoving()
				if bMoving && !self.m_bForceDeathAnim && (!self.DeathActChance || math.random(1,100) > self.DeathActChance) then
					if bRagdoll then self:SetSchedule(SCHED_DIE_RAGDOLL)
						...
						if !self.bSpecialDeath then
							timer.Simple(0.1,function()
								if !IsValid(self) then return end
								self:slvFadeOut(self:SequenceDuration() -0.1)
							end)
						end
					end
				else
	...
	elseif dmginfo:IsDamageType(DMG_DISSOLVE) then
		if self:Disposition(attacker) == D_LI then self:slvDissolve(nil, nil, 0) end
...
function ENT:EndIdlePosture()
	...
	self.bInSchedule = false
	self:SLVPlayActivity(self.m_iPostureActOut)
