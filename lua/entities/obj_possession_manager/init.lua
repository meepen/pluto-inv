...
function ENT:EndPossession()
	if IsValid(self.entPossessor) then
		...
		self.entPossessor:slvSetHealth(self.tblPlayer["Health"])
...
function ENT:Think()
	if !IsValid(self.entTarget) || !IsValid(self.entPossessor) || (!self:GetDontStopOnUse() && self.entPossessor:KeyDown(IN_USE)) || self.entPossessor:Health() <= 0 then self:EndPossession(); return end
	...
	if !self.entTarget._PossNoHealthRegen && self.nextHealthRegen && CurTime() >= self.nextHealthRegen then
		...
		if healthNew > health then
			self.entTarget:slvSetHealth(healthNew)
	...
	self.entTarget:SLVPlayActivity(act)
