...
function ENT:SetupSLVFactions()
	self:SetNPCFaction(NPC_FACTION_DWARVEN,CLASS_DWARVEN)
end
...
function ENT:OnInit()
	...
	self:slvSetHealth(GetConVarNumber("sk_centurion_sphere_health"))
...
function ENT:Equip()
	...
	self:SLVPlayActivity(ACT_SIGNAL1,false,self._PossScheduleDone)
end

function ENT:Unequip()
	if(!self:IsEquipped()) then return end
	self.m_bEquipped = false
	self:SLVPlayActivity(ACT_SIGNAL2,false,self._PossScheduleDone)
end
