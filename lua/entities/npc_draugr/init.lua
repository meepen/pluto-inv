...
function ENT:CanUseShout(shout,ent)
	...
	if(shout == SHOUT_DISARM) then
		if(ent:IsNPC() && (ent:slvClassify() == CLASS_DRAUGR || ent:slvClassify() == CLASS_FALMER)) then return false end
...
function ENT:DisarmTarget(ent)
	if(!ent:IsPlayer() && (!ent:IsNPC() || ent:slvClassify() == CLASS_DRAUGR || ent:slvClassify() == CLASS_FALMER)) then return end
	local wep = ent:GetActiveWeapon()
	...
	if(ent:IsPlayer()) then
		...
		if(ent:slvGetWeapon(class):IsValid()) then return end // Can't remove the weapon?
	else wep:Remove() end
...
function ENT:OnShoutHit(shout,ent)
	...
	if(shout == SHOUT_DISMAY) then
		...
		timer.Simple(math.Rand(6,20),function()
			...
			ent:slvAddEntityRelationship(self,D_HT,100)
		end)
		return
	end
...
function ENT:TranslateActivity(act)
	...
	if(holdType == "bow" && self.m_bBowWalk && !self:SLV_IsPossesed()) then
...
function ENT:SelectScheduleHandle(enemy,dist,distPred,disp)
	...
	if(disp == D_HT) then
		if(CurTime() >= self.m_tNextTaunt) then
			...
			if(math.random(1,3) == 1) then self:slvPlaySound("Taunt") end
		end
