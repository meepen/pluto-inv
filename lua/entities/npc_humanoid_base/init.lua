...
function ENT:OnInit()
	...
	local addon = SLVBase_Fixed.GetDerivedAddon("Skyrim")
...
function ENT:Taunt()
	...
	self:SLVPlayActivity(wep:TranslateActivity(self:SelectTauntActivity()),false,self._PossScheduleDone)
	return true
end
...
function ENT:_PossPrimaryAttack(possessor,fcDone)
	...
	if(self:IsShieldDrawn()) then self:SLVPlayActivity(ACT_SHIELD_ATTACK,false,fcDone); return end
...
function ENT:_PossJump(possessor,fcDone)
	...
	self:SLVPlayActivity(wep:TranslateActivity(actAttack),false,fcDone)
end
			...
			if(IsValid(attacker)) then
				...
				if(dotProd < 0.35) then
					if(!self:IsShieldDrawn()) then
						if(!self:SLV_IsPossesed() && CurTime() >= self.m_tNextDrawShield && math.random(1,4) <= 3) then
							self:DrawShield()
...
function ENT:HolsterWeapon(fcOnHolstered)
	...
	//if(data && data.soundUnEquip) then GAMEMODE:slvPlaySound(data.soundUnEquip,self) end
end
			...
			elseif(g != "bow_draw" && g != "bow_drawidle") then
				...
				if(hitEnt != enemy) then
					if(IsValid(hitEnt) && (hitEnt:IsNPC() || hitEnt:IsPlayer())) then
						local disp = self:slvDisposition(hitEnt)
	...
	if(self:IsShieldDrawn()) then
		...
		if(dist <= self.fMeleeDistance +10) then
			if(dist <= self.fMeleeDistance -25) then
				self:SLVPlayActivity(ACT_SHIELD_ATTACK)
				return
			end
				...
				if(self.bDirectChase && distPred <= self.fMeleeForwardDistance) then
					if(fTimeToGoal <= 0.85 && fTimeToGoal >= 0.5) then
						...
						if(math.random(1,3) >= 2) then
							self:SLVPlayActivity(wep:TranslateActivity(ACT_MELEE_ATTACK2))
							return
						end
					elseif(wep.PowerAttackSlow && fTimeToGoal <= 0.4) then
						self.m_nextChargeAttack = CurTime() +math.Rand(3,8)
						if(math.random(1,5) >= 4) then
							self:SLVPlayActivity(wep:TranslateActivity(ACT_GESTURE_MELEE_ATTACK1))
							return
						end
					end
				end
