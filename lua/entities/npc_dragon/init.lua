...
function ENT:_PossJump(entPossessor,fcDone)
	...
	if(!self:IsFlying()) then self:TakeOff()
	else self:Land() end
end
...
function ENT:OnFoundEnemy(numEnemies)
	if(!self.CanFly || self:SLV_IsPossesed()) then return end
...
function ENT:OnTakenOff()
	...
	if(!bPossessed && IsValid(self.entEnemy)) then
		...
		return
	elseif(self:IsCrippled()) then self:Land(); return end
	self:StartGliding()
...
function ENT:GetMaxSpeed() return 1000 end
...
local axisTurnSpeed = Angle(1,1,1)
...
function ENT:FindLookTarget()
	if(self:SLV_IsPossesed()) then return self:GetPossessor():GetPossessionEyeTrace().HitPos end
...
function ENT:FindFlyTarget()
	if(!self:SLV_IsPossesed()) then return self:GetFlyTarget() end
		...
		elseif(flying) then
			...
			if(self.m_bLanding && flying == FLY_GLIDE) then
				...
				if(tr.Fraction <= 0.175) then self:OnLanded()
					...
					if(!self:SLV_IsPossesed()) then ang.p = tr.Fraction *ang.p end
					self:ApproachAngle(ang)
				end
					...
					if(self.m_bGlideApproach) then
						...
						elseif(dist >= 200) then
							if(self.m_bGlideApproach == 2) then
								...
								dir.z = 0
								local dirAng = self:GetAngles():Forward()
								dirAng.z = 0
...
function ENT:OnLanded()
	...
	self:OnHitGround()
	self:SLVPlayActivity(ACT_LAND,false,self._PossScheduleDone)
end
...
function ENT:OnHitGround()
	...
	for _, ent in ipairs(ents.FindInBox(pos +min,pos +max)) do
		if(((ent:IsPlayer() || (ent:IsNPC() && ent != self)) && self:slvDisposition(ent) != D_LI) || ent:IsPhysicsEntity()) then
...
function ENT:StartGliding()
	...
	self:StopMoving()
	self:StopMoving()
	self:SLVPlayActivity(ACT_SIGNAL_ADVANCE,false,self._PossScheduleDone)
	self.m_bFlying = 2
	self.m_lastFlyStateChanged = CurTime()
	self.possOffset = self.possOffsetFlight
	if(self:SLV_IsPossesed()) then
...
function ENT:StartHovering()
	...
	self:StopMoving()
	self:StopMoving()
	self:SLVPlayActivity(ACT_SIGNAL_HALT,false,self._PossScheduleDone)
	self.m_bFlying = 1
	self.m_lastFlyStateChanged = CurTime()
...
function ENT:EventHandle(...)
	...
	if(event == "inhale") then
		if(self.m_rattackShout && self.m_rattackShout != SHOUT_FROST_BREATH && self.m_rattackShout != SHOUT_FIRE_BREATH) then self:slvPlaySound("Shout" .. self.m_rattackShout .. "a") end
		return true
	end
	if(event == "exhale") then
		...
		self:SLVPlayActivity(self:SelectShoutActivity(self.m_rattackShout),!self:IsFlying(),self:SLV_IsPossesed() && self._PossScheduleDone)
		return true
	end
	...
	if(event == "crash") then
		local type = select(2,...)
		if(type == "long") then
			self:SLVPlayActivity(ACT_SIGNAL3,false,self._PossScheduleDone) -- We're done crashing. Standing up
		else	-- Short crash; We just hit the ground
			self:OnCrash()
			self:SLVPlayActivity(ACT_SIGNAL_LEFT,false,self._PossScheduleDone)
		end
		return true
	end
	if(event == "shout") then
		...
		self:slvPlaySound("Shout" .. self.m_rattackShout .. "b")
		local pos = att.Pos
		local ang = att.Ang
		if(IsValid(self.entEnemy)) then
...
function ENT:InitSandbox()
	...
	if(math.Rand(0,1) <= f && !SLVBase_Fixed.IsSoundtrackNPCActive(self)) then self:PlaySoundtrack() end
end
...
function ENT:DisarmTarget(ent)
	...
	if wep then
		wep:SetPos(pos +dir *60)
		wep:SetAngles(dir:Angle())
		wep:SetKeyValue("spawnflags","2") // Disable player pickup
		wep:Spawn()
		wep:Activate()
	end
	timer.Simple(0.5,function()
		if(wep:IsValid()) then
			wep:SetKeyValue("spawnflags","0") // Enable player pickup
		end
	end)
	local phys = wep:GetPhysicsObject()
	if(!phys:IsValid()) then wep:DropToFloor()
...
function ENT:OnHit(ent)
	...
	if(shout == SHOUT_FIRE_BREATH) then
		ent:slvIgnite(ent:IsPlayer() && 2 || 8,0)
		local dmginfo = DamageInfo()
	...
	if(shout == SHOUT_DISMAY) then
		if(ent:IsNPC()) then
			...
			ent:slvAddEntityRelationship(self,D_FR,100)
			return
		end
	end
	...
	if(shout == SHOUT_FIREBALL) then
		ent:slvIgnite(ent:IsPlayer() && 6 || 12,0)
		local dmginfo = DamageInfo()
		dmginfo:SetDamageType(DMG_SLOWBURN)
		dmginfo:SetDamage(GetConVarNumber("sk_" .. self.skName .. "_dmg_flame") *2)
		if(ent:IsPlayer()) then dmginfo:ScaleDamage(0.5) end
...
function ENT:Crash(bForceLong)
	...
	if(!bForceLong && pos.z -tr.HitPos.z <= 1000) then	-- Close enough to the ground to do a short crash
		self:SLVPlayActivity(ACT_SIGNAL_RIGHT)
		return
	end
	self.m_bLongCrash = true
	self:SLVPlayActivity(ACT_SIGNAL1) -- TODO: Make sure all subsequent animations are playing when possessed
end
...
function ENT:SelectShout()
	...
	if(r == 3 || #shouts == 0) then
		...
		if(self:CanUseShout(SHOUT_DISMAY,ent) && (!valid || (ent:IsNPC() && ent:slvDisposition(self) != D_FR))) then table.insert(shouts,SHOUT_DISMAY) end
...
function ENT:UseShout()
	...
	self:StopMoving()
	self:StopMoving()
	self:SLVPlayActivity(self:SelectRangeActivity(),!self:IsFlying() && !self:SLV_IsPossesed())
end
	...
	if(bFlying || self:Visible(enemy)) then
		if(!bFlying) then	-- We're on the ground doing our usual stuff
			...
			if(dist <= self.fMeleeDistance || distPred <= self.fMeleeDistance) then
				self:SLVPlayActivity(self:SelectMeleeActivity())
				return
			end
