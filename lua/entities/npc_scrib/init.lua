...
function ENT:SetupSLVFactions()
	self:SetNPCFaction(NPC_FACTION_SCRIB,CLASS_SCRIB)
end
...
function ENT:UnBurrow()
	...
	self.m_cspIdle:Play()
	self:CallOnInitialized(function() self:SLVPlayActivity(ACT_SIGNAL2,false,self._PossScheduleDone) end)
	self.m_unburrow = nil
...
function ENT:Burrow(tmUnburrow)
	...
	self:CallOnInitialized(function() self:SLVPlayActivity(ACT_SIGNAL1,false,self._PossScheduleDone) end)
end
