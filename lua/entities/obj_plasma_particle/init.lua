...
function ENT:Initialize()
	...
	entLight:SetKeyValue("_light", "0 200 0 100")
	...
	util.ParticleEffect("plasma_projectile_trail", self:GetPos(), self:GetAngles(), self, nil, false)
...
function ENT:PhysicsCollide(data, physobj)
	...
	if data.HitEntity:IsPlayer() || data.HitEntity:IsNPC() then
		...
		if sClass != "npc_turret_floor" then
			local iDmg = self.dmg || GetConVarNumber("sk_mistergutsy_dmg_plasma")
			local dmg = DamageInfo()
			dmg:SetDamage(iDmg)
			dmg:SetAttacker(self:GetEntityOwner())
			dmg:SetInflictor(self)
